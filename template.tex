...
$endif$
% Package float pour forcer la position des images
\usepackage{float}
% Redéfinir le placement par défaut des figures pour utiliser H (Here, exactement ici)
\makeatletter
\def\fps@figure{H}
\makeatother
...
% Code inline avec fond coloré
% Permet les retours à la ligne automatiques pour le code long
\let\oldtexttt\texttt
\renewcommand{\texttt}[1]{%
  \begingroup%
  \ttfamily\small%
  \fboxsep=2pt\fboxrule=0pt%
...
% Configuration du positionnement des images
% Si fix-images est défini à true, les images restent exactement à leur position
% Sinon, LaTeX les positionne automatiquement (comportement par défaut)
$if(fix-images)$
\makeatletter
% Redéfinir l'environnement figure pour toujours utiliser [H]
